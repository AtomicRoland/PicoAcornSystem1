...
parameter:
	...
	Color_Foreground : Select the foreground color of the English character
******************************************************************************/
void GUI_DisCharH(POINT Xpoint, POINT Ypoint, const char Acsii_Char,
                 sFONT* Font, COLOR Color_Background, COLOR Color_Foreground)
{
    POINT Page, Column;

    if(Ypoint > sLCD_DIS.LCD_Dis_Column || Xpoint > sLCD_DIS.LCD_Dis_Page) {
        //DEBUG("GUI_DisChar Input exceeds the normal display range\r\n");
        return;
    }
    ...
    for(Page = 0; Page < Font->Height; Page ++ ) {
        for(Column = 0; Column < Font->Width; Column ++ ) {
            ...
            if(FONT_BACKGROUND == Color_Background) { //this process is to speed up the scan
                if(*ptr & (0x80 >> (Column % 8)))
                    GUI_DrawPoint(Ypoint + Page, Xpoint + Font->Width - Column, Color_Foreground, DOT_PIXEL_DFT, DOT_STYLE_DFT);
            } else {
                if(*ptr & (0x80 >> (Column % 8))) {
                    GUI_DrawPoint(Ypoint + Page, Xpoint + Font->Width - Column, Color_Foreground, DOT_PIXEL_DFT, DOT_STYLE_DFT);
                } else {
                    GUI_DrawPoint(Ypoint + Page, Xpoint + Font->Width - Column, Color_Background, DOT_PIXEL_DFT, DOT_STYLE_DFT);
                }
            }
            //One pixel is 8 bits
            if(Column % 8 == 7)
